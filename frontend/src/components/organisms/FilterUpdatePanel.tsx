...
import { useUpdateFilterMutation } from "../../app/store/filter/filter.api";
...
export const FilterUpdatePanel = (props: {
    filter: Filter
    onClose: () => void
}) => {
    ...
    const [state, setState] = useState<Filter>(props.filter);
    ...
    const onSubmit = () => {
        ...
        .then(() => {
            notifyInfo(t('update_filter_success', {filterName: state.name}));
            props.onClose();
        })
        .catch((err) => {
