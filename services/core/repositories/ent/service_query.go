...
func (sq *ServiceQuery) QueryPlugins() *PluginQuery {
...
func (sq *ServiceQuery) QueryWorkspaces() *WorkspaceQuery {
...
func (sq *ServiceQuery) QueryFilters() *FilterQuery {
	query := (&FilterClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *ServiceQuery) QuerySessions() *SessionQuery {
...
func (sq *ServiceQuery) Clone() *ServiceQuery {
	...
	return &ServiceQuery{
		...
		withPlugins:    sq.withPlugins.Clone(),
		withWorkspaces: sq.withWorkspaces.Clone(),
		withFilters:    sq.withFilters.Clone(),
		withSessions:   sq.withSessions.Clone(),
...
func (sq *ServiceQuery) WithPlugins(opts ...func(*PluginQuery)) *ServiceQuery {
	query := (&PluginClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withPlugins = query
	return sq
}
...
func (sq *ServiceQuery) WithWorkspaces(opts ...func(*WorkspaceQuery)) *ServiceQuery {
	query := (&WorkspaceClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withWorkspaces = query
	return sq
}
...
func (sq *ServiceQuery) WithFilters(opts ...func(*FilterQuery)) *ServiceQuery {
	query := (&FilterClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *ServiceQuery) WithSessions(opts ...func(*SessionQuery)) *ServiceQuery {
	query := (&SessionClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withSessions = query
	return sq
}
...
func (sq *ServiceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Service, error) {
	...
	if query := sq.withPlugins; query != nil {
		if err := sq.loadPlugins(ctx, query, nodes,
			func(n *Service) { n.Edges.Plugins = []*Plugin{} },
			func(n *Service, e *Plugin) { n.Edges.Plugins = append(n.Edges.Plugins, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withWorkspaces; query != nil {
		if err := sq.loadWorkspaces(ctx, query, nodes,
			func(n *Service) { n.Edges.Workspaces = []*Workspace{} },
			func(n *Service, e *Workspace) { n.Edges.Workspaces = append(n.Edges.Workspaces, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withFilters; query != nil {
		if err := sq.loadFilters(ctx, query, nodes,
			func(n *Service) { n.Edges.Filters = []*Filter{} },
			func(n *Service, e *Filter) { n.Edges.Filters = append(n.Edges.Filters, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withSessions; query != nil {
		if err := sq.loadSessions(ctx, query, nodes,
			func(n *Service) { n.Edges.Sessions = []*Session{} },
			func(n *Service, e *Session) { n.Edges.Sessions = append(n.Edges.Sessions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ServiceQuery) loadPlugins(ctx context.Context, query *PluginQuery, nodes []*Service, init func(*Service), assign func(*Service, *Plugin)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(service.PluginsTable)
		s.Join(joinT).On(s.C(plugin.FieldID), joinT.C(service.PluginsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(service.PluginsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(service.PluginsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Plugin](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
...
func (sq *ServiceQuery) loadWorkspaces(ctx context.Context, query *WorkspaceQuery, nodes []*Service, init func(*Service), assign func(*Service, *Workspace)) error {
	...
	query.Where(predicate.Workspace(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(service.WorkspacesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.service_workspaces
		if fk == nil {
			return fmt.Errorf(`foreign-key "service_workspaces" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "service_workspaces" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (sq *ServiceQuery) loadFilters(ctx context.Context, query *FilterQuery, nodes []*Service, init func(*Service), assign func(*Service, *Filter)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Service)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(filter.FieldServiceID)
	}
	query.Where(predicate.Filter(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(service.FiltersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (sq *ServiceQuery) loadSessions(ctx context.Context, query *SessionQuery, nodes []*Service, init func(*Service), assign func(*Service, *Session)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Service)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Session(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(service.SessionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.service_sessions
		if fk == nil {
			return fmt.Errorf(`foreign-key "service_sessions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "service_sessions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
